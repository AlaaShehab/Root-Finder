...
switch popup_sel_index
    ...
    case 7
        plottingBisection(handles,  result.xi2, result.xio, result.xi1, result.f, result.itr, step, type);
end
...
switch popup_sel_index
    ...
    case 2
        ...
        bvo = output(aArray, bArray, cArray, f, root, itr);
         if errorMsg ~= "VALID"
                 msgbox(char(errorMsg));
            return;
         end
        setTextField(handles, error(itr), root, time, itr);
    case 3
		[root, xls, xus, xrs, itr, error, errorMsg, time] = bisection(f,x0, x1, Es, MaxItr);
        o = output([], [], f, xls, xrs,xus,root,error, itr);
         if errorMsg ~= "VALID"
                 msgbox(char(errorMsg));
            return;
         end
